...
#define ESP_WIFI_SSID "ubilab_wifi"
#define ESP_WIFI_PASS "ohg4xah3oufohreiPe7e"
...
esp_err_t jpg_stream_httpd_handler(httpd_req_t *req){
    ...
    int64_t fr_next = esp_timer_get_time();
    const int64_t fr_time = 200 * 1000;

    while(true){
        int64_t fr_now = esp_timer_get_time();
        int64_t diff = fr_next - fr_now;
        //ESP_LOGI(TAG, "now: %lld last: %lld diff: %lld period: %d", fr_now, fr_last, diff, 1000 * portTICK_PERIOD_MS);
        if (diff > 0) {
          vTaskDelay(diff / (1000 * portTICK_PERIOD_MS));
          fr_next = fr_next + fr_time;        
        } else {
          fr_next = fr_now + fr_time;        
        }
